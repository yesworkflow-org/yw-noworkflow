...
banner('(StepName,Description) % What workflow steps comprise the top-level workflow?').
[user].
:- table yw_q2/2.
...
banner('(SourceFile, StartLine, EndLine) % Where is the definition of workflow step HelloWorld.print_greeting?').
[user].
:- table yw_q3/3.
...
banner('(OutputName, Description) % What are the names and descriptions of any outputs of the script?').
[user].
:- table yw_q4/2.
...
banner('(FunctionName, CallLine) % What functions are called from the top level of the script?').
[user].
:- table nw_q1/1.
nw_q1(FunctionName) :-
...
banner('(ActivationId, ParameterName, VariableName, Value) % What variable values are passed to print_greeting() from the top of the script?').
[user].
:- table nw_q2/4.
...
banner('(ActivationId, Parameter, Value) % What literal values are passed to print_greeting() from the top of the script?').
[user].
:- table nw_q3/3.
